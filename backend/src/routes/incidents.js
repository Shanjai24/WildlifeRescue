...
import { Incident, IncidentStatusHistory, Notification } from '../lib/db.js';
...
router.post('/', auth(), role('animal_lover'), async (req, res) => {
  try {
    ...
    if (!animalCategory || !incidentType || !description) return res.status(400).json({ error: 'Missing fields' });
    ...
    await IncidentStatusHistory.create({ incidentId: incident.id, status: 'reported', changedByUserId: req.user.id });
...
router.get('/', auth(), role('animal_lover'), async (req, res) => {
  try {
    const incidents = await Incident.findAll({ where: { reporterId: req.user.id }, order: [['createdAt', 'DESC']] });
    res.json(incidents);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
  }
